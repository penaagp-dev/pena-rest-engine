...
class Boto3(object):
    ...
    def connect(self):
        ...
        requested_services = set(
            svc.lower() for svc in self.app.config.get('BOTO3_SERVICES', [])
        )

        region = self.app.config.get('BOTO3_REGION')
        sess_params = {
            'aws_access_key_id': self.app.config.get('BOTO3_ACCESS_KEY'),
            'aws_secret_access_key': self.app.config.get('BOTO3_SECRET_KEY'),
            'profile_name': self.app.config.get('BOTO3_PROFILE'),
            'region_name': region
        }
